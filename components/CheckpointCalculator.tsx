...
import { calculateCheckpoints, Checkpoint as CheckpointType } from "../utils/timeCalculator.ts";
...
export default function CheckpointCalculator({ data, basePaceValue }: CheckpointCalculatorProps) {
    ...
    if (savedCheckpoints) {
      try {
        ...
        if (Array.isArray(parsedCheckpoints)) {
          ...
          const validCheckpoints = parsedCheckpoints.filter(cp => cp <= totalDistance);
  ...
  useEffect(() => {
    if (basePaceValue && customCheckpoints.length > 0) {
      const calculatedCheckpoints = calculateCheckpoints(data, basePaceValue, customCheckpoints);
  ...
  const handleAddSinglePoint = () => {
    ...
    try {
      ...
      if (pointDistance > totalDistance) {
        setErrorMessage(`Il punto ${pointDistance} km supera la lunghezza del percorso (${totalDistance.toFixed(2)} km)`);
        return;
      }
      ...
      const newCheckpoints = [...customCheckpoints, pointDistance].sort((a, b) => a - b);
  ...
  const handleRemoveCheckpoint = (checkpoint: number) => {
    const updatedCheckpoints = customCheckpoints.filter(cp => cp !== checkpoint);
    ...
    <div className="mt-2">
      ...
      <div className="mt-2 mb-4">
        <p className="text-sm text-gray-600 mb-2">
          Aggiungi punti intermedi per vedere i tempi previsti lungo il percorso.
        </p>
          ...
          <div className="flex flex-wrap items-end gap-2 p-3 bg-blue-50 rounded-lg">
            <div className="flex-grow">
              ...
              <input
                type="text"
                value={newPointKm}
                onInput={(e) => setNewPointKm((e.target as HTMLInputElement).value)}
                placeholder="5.5"
            ...
            <div className="flex gap-2">
              <Button onClick={handleAddSinglePoint} disabled={!basePaceValue || !newPointKm.trim()}>
                Aggiungi
              </Button>
              <button 
                onClick={() => {
      ...
      {checkpoints.length > 0 ? (
        <div className="overflow-x-auto">
          <table className="min-w-full bg-white border border-gray-200 rounded-lg">
            <thead className="bg-gray-50">
              <tr>
                <th className="py-2 px-3 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Km</th>
                <th className="py-2 px-3 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Altitudine</th>
                <th className="py-2 px-3 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Terreno</th>
                <th className="py-2 px-3 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ritmo</th>
                <th className="py-2 px-3 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tempo previsto</th>
                <th className="py-2 px-3 border-b text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Azioni</th>
              </tr>
            </thead>
              ...
              {checkpoints.map((checkpoint) => {
                ...
                return (
                  <tr key={checkpoint.distance} className={rowBgColor}>
                    <td className="py-3 px-3">{checkpoint.distance.toFixed(2)} km</td>
                    <td className="py-3 px-3">{Math.round(checkpoint.elevation)} m</td>
                    <td className="py-3 px-3">
                      {checkpoint.segment}
                      {checkpoint.grade !== undefined && checkpoint.grade !== 0 && (
                        <span className="ml-1 text-xs text-gray-500">
                          ({checkpoint.grade > 0 ? '+' : ''}{checkpoint.grade.toFixed(1)}%)
                        </span>
                      )}
                    </td>
                    <td className="py-3 px-3">{checkpoint.formattedPace}</td>
                    <td className="py-3 px-3 font-medium">{checkpoint.formattedTime}</td>
                    <td className="py-3 px-3">
                      {checkpoint.segment !== "Arrivo" && (
                        <button
                          onClick={() => handleRemoveCheckpoint(checkpoint.distance)}
        ...
        <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-lg">
          <p className="text-gray-500">
            Nessun punto intermedio definito. Clicca sul pulsante + per aggiungere punti.
          </p>
        </div>
      )}
    </div>
